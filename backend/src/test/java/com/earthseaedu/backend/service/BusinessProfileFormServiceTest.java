...
class BusinessProfileFormServiceTest {
    ...
    private BusinessProfileFormMapper mapper;
    private BusinessProfileFormService service;

    @BeforeEach
    void setUp() {
        mapper = mock(BusinessProfileFormMapper.class);
    ...
    @Test
    void loadBusinessProfileSnapshotUsesJoinedQueryForLanguageScoreItems() {
        given(mapper.countTable("student_language_test_score_item")).willReturn(1);
        given(mapper.showColumns("student_language_test_score_item")).willReturn(List.of(
            column("student_language_test_record_id", "bigint"),
            column("score_item_code", "varchar(32)"),
            column("score_value", "decimal(10,2)")
        ));
        given(mapper.selectLanguageTestScoreItemRows(anyList(), anyString())).willReturn(List.of(
            Map.of(
                "student_language_test_record_id", 101L,
                "score_item_code", "LISTENING",
                "score_value", 7.5
            )
        ));

        Map<String, Object> snapshot = service.loadBusinessProfileSnapshot("stu-1");

        assertThat(castList(snapshot.get("student_language_test_score_item"))).hasSize(1);
        verify(mapper).selectLanguageTestScoreItemRows(anyList(), anyString());
    }
