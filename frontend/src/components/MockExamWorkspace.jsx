...
export default function MockExamWorkspace({
  ...
  useEffect(() => {
    ...
    async function bootstrapPage() {
      try {
        ...
        setMessage(
          toMessage("error", error?.response?.data?.detail || "模拟考试页面初始化失败，请稍后重试。")
        );
      } finally {
        if (active) {
  ...
  useEffect(() => {
    ...
    async function loadSelectorResources() {
      ...
      try {
        ...
        setSelectorForm((previous) => ({
          ...previous,
          question_bank_id: nextQuestionBanks.some(
            (item) => String(item.id) === String(previous.question_bank_id)
          )
            ? previous.question_bank_id
            : nextQuestionBanks[0]
              ? String(nextQuestionBanks[0].id)
              : "",
          exam_set_id: nextEnabledExamSets.some(
            (item) => String(item.exam_sets_id) === String(previous.exam_set_id)
          )
        ...
        if (!(options.supported_categories || []).includes(selectorForm.exam_category)) {
          setMessage(
            toMessage(
              "warn",
              `${selectorForm.exam_category} 模考内容还在整理中，当前请先使用其他已上线题库。`
            )
          );
          return;
        }
        ...
        if (selectorForm.source_kind === "exam-set" && nextEnabledExamSets.length === 0) {
          setMessage(toMessage("warn", "当前筛选条件下没有可用试卷，请先创建并启用组合试卷。"));
          return;
        }
        ...
        setExamSets([]);
        setMessage(
          toMessage("error", error?.response?.data?.detail || "模拟考试资源加载失败，请稍后重试。")
        );
      } finally {
        if (active) {
    ...
    selectorForm.exam_category,
    selectorForm.exam_content,
    selectorForm.source_kind,
  ]);

  useEffect(() => {
    if (!showExamSetManagement || loadingBootstrap || !createForm.exam_category) {
      return;
    }
  ...
  }, [createForm.exam_category, loadingBootstrap, showExamSetManagement]);
  ...
  function handleStartExam() {
    ...
    if (!isSupportedSelectorCategory) {
      setMessage(
        toMessage("warn", `${selectorForm.exam_category} 模考内容还在整理中，请先使用其他已上线题库。`)
      );
      return;
    }
  ...
  async function handleCreateExamSet() {
    ...
    if (createForm.mode === "random" && createForm.exam_contents.length === 0) {
      setCreateMessage("随机组卷至少选择一个考试内容。");
      return;
    }
  ...
  async function handleBuildQuickPractice() {
    ...
    if (!(options.supported_categories || []).includes(quickPracticeForm.exam_category)) {
      setQuickPracticeMessage(
        `${quickPracticeForm.exam_category} 模考内容还在整理中，请先使用其他已上线题库。`
      );
      return;
    }
  ...
  function handleStartExamSet(item) {
    ...
    if (!(options.supported_categories || []).includes(item.exam_category)) {
      setMessage(
        toMessage("warn", `${item.exam_category} 模考内容还在整理中，请先使用其他已上线题库。`)
      );
      return;
    }
  ...
  return (
    <div className="mockexam-module-stack">
      <motion.section
      ...
      >
        ...
        <div className="mockexam-segmented">
          <button
            ...
            className={
              selectorForm.source_kind === "question-bank"
                ? "mockexam-segmented-button active"
                : "mockexam-segmented-button"
            }
            onClick={() =>
              setSelectorForm((previous) => ({ ...previous, source_kind: "question-bank" }))
            }
          >
            选择题目
          </button>
          <button
            type="button"
            className={
              selectorForm.source_kind === "exam-set"
                ? "mockexam-segmented-button active"
                : "mockexam-segmented-button"
            }
            onClick={() =>
              setSelectorForm((previous) => ({ ...previous, source_kind: "exam-set" }))
            }
          >
            选择试卷
          </button>
      ...
      {!showExamSetManagement && showQuickPractice ? (
        <motion.section
          className="mockexam-panel"
          initial={{ opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className="mockexam-manage-grid">
            ...
            <div className="mockexam-field">
              ...
              <input
                ...
                onChange={(event) =>
                  setQuickPracticeForm((previous) => ({ ...previous, count: event.target.value }))
                }
              />
            </div>
          </div>
          ...
          <div className="mockexam-chip-grid">
            {quickPracticeContentOptions.map((item) => (
              <button
                ...
                type="button"
                className={
                  quickPracticeForm.exam_contents.includes(item)
                    ? "mockexam-chip-button active"
                    : "mockexam-chip-button"
                }
                onClick={() =>
                  setQuickPracticeForm((previous) => ({
                    ...previous,
          ...
          <div className="mockexam-inline-actions">
            <button
              ...
              disabled={loadingQuickPractice}
              onClick={handleBuildQuickPractice}
            >
                  ...
                  <div className="mockexam-chip-grid">
                    {createContentOptions.map((item) => (
                      <button
                        ...
                        type="button"
                        className={
                          createForm.exam_contents.includes(item)
                            ? "mockexam-chip-button active"
                            : "mockexam-chip-button"
                        }
                        onClick={() =>
                  ...
                  <div className="mockexam-manage-grid">
                    <div className="mockexam-field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCreateForm((previous) => ({ ...previous, per_content: event.target.value }))
                        }
                      />
                    </div>
                    <div className="mockexam-field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCreateForm((previous) => ({ ...previous, extra_count: event.target.value }))
                        }
                      />
                    </div>
                    <div className="mockexam-field">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setCreateForm((previous) => ({ ...previous, total_count: event.target.value }))
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div key={item.exam_sets_id} className="mockexam-set-card">
                    <div className="mockexam-set-head">
                      ...
                      <span
                        className={
                          item.status === "1"
                            ? "mockexam-status-chip active"
                            : "mockexam-status-chip"
                        }
                      >
