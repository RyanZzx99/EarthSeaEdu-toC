...
import { getMockExamWrongQuestions } from "../api/mockexam";
...
export default function MockExamWrongBookPanel() {
  ...
  useEffect(() => {
    ...
    async function loadItems() {
      try {
        setLoading(true);
        const response = await getMockExamWrongQuestions({ limit: 20 });
        if (!active) {
          return;
        }
        ...
        setItems([]);
        setMessage(getApiError(error, "错题本加载失败"));
      } finally {
        if (active) {
          setLoading(false);
          setLoadedOnce(true);
        ...
        <>
          {loading ? (
            <div className="mockexam-inline-note">
              <LoaderCircle size={16} strokeWidth={2.1} className="spin" /> 正在加载错题本...
            </div>
          ) : null}
          ...
          {!loading && !message ? (
            <div className="mockexam-history-list">
              {items.map((item, index) => (
                <article
                  key={`${item.exam_question_id || item.question_id}-${index}`}
                  className="mockexam-history-card"
                >
                  <div className="mockexam-history-copy">
                    <div className="mockexam-history-tags">
                      ...
                      <span>错 {item.wrong_count || 0} 次</span>
                    </div>
                    <h4>{item.paper_title}</h4>
                    <p>{item.preview_text || item.question_id}</p>
                    <p>{formatTime(item.last_wrong_time)}</p>
                  </div>
                  ...
                  <div className="mockexam-history-actions">
                    <button
                      ...
                      disabled={!item.exam_question_id}
              ...
              {!items.length ? <div className="mockexam-beta-empty">暂无错题记录</div> : null}
            </div>
          ) : null}
        </>
      ) : null}
